    ...
    try{
        await getUserData(req.params.pin).then(profileData => {
            if(profileData !== null){
                ...
                res.status(400).json({
                    error: "There is any user with this pin"
                })
            }
        });
    }
...
async function getUserData(userPin) {
    let profileData = { name: "", email: "", image: "", currentWeekAverage: null, lastWeekAverage: null}
    ...
    if(user !== null){
        ...
        if(checkinsCurrentWeek.length > 0){
            profileData.currentWeekAverage = (checkinsCurrentWeek[0].totalMinutes / checkinsCurrentWeek[0].numberOfCheckins);
        }

        if(checkinsLastWeek.length > 0){
            profileData.lastWeekAverage = (checkinsLastWeek[0].totalMinutes / checkinsLastWeek[0].numberOfCheckins);
        }
    }
    return profileData;
}